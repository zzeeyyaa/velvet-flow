...
const types_1 = require("../RESP/types");
...
class RedisSentinel extends node_events_1.EventEmitter {
    ...
    async *scanIterator(options) {
        ...
        try {
            do {
                if (masterChanged)
                    throw new errors_1.ScanIteratorInterruptedError();
                ...
                let reply;
                try {
                    reply = await this._execute(false, client => {
                    ...
                    if (err instanceof errors_1.ScanIteratorInterruptedError)
                        throw err;
                    ...
                    if (masterChanged)
                        throw new errors_1.ScanIteratorInterruptedError(err);
                    throw err;
                }
                cursor = reply.cursor;
                yield reply.keys;
...
class RedisSentinelInternal extends node_events_1.EventEmitter {
    ...
    async transform(analyzed) {
        ...
        const mergedSentinelList = (0, utils_1.mergeSentinelNodes)(this.#sentinelSeedNodes, analyzed.sentinelList);
